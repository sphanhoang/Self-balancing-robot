...
<body>
    ...
    <div class="panel">
        <h3>&#x2699; Roll PID Tuning</h3>
        <div class="slider-container">
            KP: <input type="range" id="kpRoll" min="0" max="50" step="0.2" class="pid-slider">
            <span id="kpRollValue" class="value-display">20.0</span>
        </div>
        <div class="slider-container">
            KI: <input type="range" id="kiRoll" min="0" max="50" step="0.2" class="pid-slider">
            <span id="kiRollValue" class="value-display">2.0</span>
        </div>
        <div class="slider-container">
            KD: <input type="range" id="kdRoll" min="0" max="10" step="0.1" class="pid-slider">
    ...
    <div class="panel">
        <h3>&#x1F3AE; Control</h3>
        <div class="slider-container">
            Tilt Angle: <input type="range" id="target_angle" min="-2" max="2" step="0.01" class="pid-slider">
    ...
    <script>
        ...
        function initSliders() {
            document.querySelectorAll('input[type="range"]').forEach(slider => {
                ...
                slider.addEventListener('input', function() {
                    ...
                    updateTimer = setTimeout(updatePID, 100);
                });
            });
        }
        ...
        function updatePID() {
            ...
            fetch('/pid?' + params)
                .then(r => r.text())
                .then(data => {
                    console.log('PID update response:', data);
                })
                .catch(err => console.error('PID update error:', err));
        }

         // Reset to default parameters
        function resetParameters() {
            document.getElementById('kpRoll').value = 25.0;
            document.getElementById('kiRoll').value = 4.0;
            document.getElementById('kdRoll').value = 0.8;
            document.getElementById('kpSpeed').value = 0.01;
            document.getElementById('kiSpeed').value = 0.01;
            document.getElementById('kdSpeed').value = 0.01;
            document.getElementById('target_angle').value = 0.5;
...
void wifi_init(void) 
{
    ...
    {
        // NVS partition is corrupted, needs to be erased
